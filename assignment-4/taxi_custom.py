...
class TaxiCustomEnv(Env):
    ...
    * v0: Initial versions release
    ...
    @staticmethod
    def find_locations(map_repr):
        map_cur = map_repr[1:-1]
        pattern = r'[+\-|:]'
        filtered_map = [re.sub(pattern, '', row) for row in map_cur]
        locations = []
        for char in ['R', 'G', 'Y', 'B']:
            for row_index, row in enumerate(filtered_map):
                for match in re.finditer(char, row):
                    locations.append((row_index, match.start()))
                    break
        return locations

    def __init__(self, render_mode: Optional[str] = None, size=5):
        self.size = size
        self.desc = np.asarray(MAPS[size], dtype="c")

        self.locs = locs = TaxiCustomEnv.find_locations(MAPS[size])

        self.locs_colors = [(255, 0, 0), (0, 255, 0), (255, 255, 0), (0, 0, 255)]

        num_states = size * size * 5 * 4
        num_rows = size
        num_columns = size
        max_row = num_rows - 1
    ...
    #     encoded_state = taxi_row + R * (taxi_col[1] + C * (pass_loc[2] + P * state[3]))
    #     return encoded_state
    #
    # def decode_state(selfm, encoded_state, R, C, P, D):
    #     d = encoded_state % D
    #     p = ((encoded_state - d) // D) % P
    #     c = (((encoded_state - d) // D) - p) // P % C
    #     r = ((((encoded_state - d) // D) - p) // P - c) // C % R
    #     return r, c, p, d
    ...
    def encode(self, taxi_row, taxi_col, pass_loc, dest_idx):
        ...
        i *= 4
        i += dest_idx
        assert 0 <= i < self.size * self.size * 5 * 4
        return i
    ...
    def action_mask(self, state: int):
        ...
        if taxi_row < self.size - 1:
            mask[0] = 1
        if taxi_row > 0:
            mask[1] = 1
        if taxi_col < self.size - 1 and self.desc[taxi_row + 1, 2 * taxi_col + 2] == b":":
            mask[2] = 1
