...
class OneMaxGenerator:
    ...
    def generate(seed, size):
        np.random.seed(seed)
        problem = ml_h.DiscreteOpt(length=size, fitness_fn=ml_h.fitness.OneMax())
        return problem


problem_name = 'one-max'
problem_sizes = [10, 50, 250]
problem_small = OneMaxGenerator().generate(SEED, size=problem_sizes[0])
problem_medium = OneMaxGenerator().generate(SEED, size=problem_sizes[1])
problem_big = OneMaxGenerator().generate(SEED, size=problem_sizes[2])
...
def run_sa_all():
    ...
    for i in range(len(problems)):
        ...
        run_sa_algorithm(problem=problems[i],
                         ...
                         max_attempts=300, iterations=ITERATIONS)
