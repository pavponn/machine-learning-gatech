...
class MyQueensOpt(ml_h.QueensOpt):
    pass
    # def can_stop(self):
    #     return False


class MyQueensGenerator(ml_h.QueensGenerator):
    @staticmethod
    def generate(seed, size=20):
        np.random.seed(seed)
        problem = MyQueensOpt(length=size)
        return problem


problem_name = 'queens'
problem_sizes = [10, 25, 50]
problem_small = MyQueensGenerator().generate(SEED, size=problem_sizes[0])
problem_medium = MyQueensGenerator().generate(SEED, size=problem_sizes[1])
problem_big = MyQueensGenerator().generate(SEED, size=problem_sizes[2])
